...
*Version 3.0 — March 2026 | Status: Architectural Specification (Pre-Implementation)*
...
Additionally, integration crates **tk-dmrg** and **tk-dmft** implement the full DMRG sweep algorithm, iterative eigensolvers (Lanczos/Davidson, written in-house), MPO compilation from `OpSum`, time-evolution methods (TDVP with Tikhonov-regularized gauge shifts and site-tensor subspace expansion for entanglement growth, TEBD as fallback), and the DMFT self-consistency loop. A **tk-python** crate provides PyO3 bindings with explicit GIL management, thread-safe `AtomicBool` cancellation, and zero-copy NumPy interop.
...
|:------|:---------------|:-----------|
| **tk-core** | Tensor shape/stride metadata, contiguous storage buffers, `MatRef`/`MatMut` with lazy conjugation flag, arena allocators (with pinned-memory budget tracking), TensorCow (Copy-on-Write), element-type generics, error types | *(none — leaf crate)* |
...
| **tk-contract** | ContractionGraph DAG, PathOptimizer trait, greedy/TreeSA heuristics, ContractionExecutor with reshape-free GEMM, conjugation flag propagation | tk-core, tk-symmetry, tk-linalg |
...
| **tk-python** | PyO3/maturin bindings with explicit GIL release, `AtomicBool`-based cancellation (no GIL re-acquisition from Rayon workers), zero-copy NumPy via rust-numpy | tk-dmft |
...
#### 3.3.2 Copy-on-Write (Cow) Semantics
...
**Thread-safe cancellation via `AtomicBool`:** The original v2 design proposed re-acquiring the GIL from within Rayon worker threads via `Python::with_gil`. This is unsound: Rayon worker threads are not registered with the CPython interpreter, and attempting to acquire the GIL from an unregistered thread while the main Python thread is blocked inside `allow_threads` can cause deadlocks or segfaults. The corrected design uses an `AtomicBool` flag as a decoupled cancellation channel:
...
impl PyDmftLoop {
    pub fn solve(&mut self, py: Python<'_>) -> PyResult<PySpectralFunction> {
        ...
        let result = py.allow_threads(move || {
            // Spawn the solver on the current (main) thread.
            // Rayon workers check the AtomicBool — they never touch the GIL.
            match &mut self.inner {
                DmftLoopVariant::RealU1(solver) => {
                    solver.solve_with_cancel_flag(&flag_clone)
                }
                // ...
            }
        });

        // Back on the main thread with GIL held: check for cancellation.
...
For interactive Jupyter usage, a separate monitoring thread (spawned by Python, not by Rayon) periodically re-acquires the GIL to check for `SIGINT`:

```rust
// Spawned from Python before calling solve():
...
This design guarantees that GIL re-acquisition only happens on a thread that Python controls — never from a Rayon worker.
...
1. At the `BondCentered` step, compute the one-site Hamiltonian residual projected onto the null space of the current bond: P_null = (I - A_L · A_L†) · H_MPO · |ψ_center⟩.
2. Extract the dominant left null-space vectors from P_null via SVD, retaining D_expand vectors with the largest singular values.
3. Pad the site tensor A_L (or A_R) by appending these D_expand new basis vectors as additional columns, increasing its bond dimension from D to D + D_expand. The new columns span directions orthogonal to the current tangent space.
4. Pad the bond matrix C with zeros along the corresponding new rows/columns. The expanded C is now (D + D_expand) × (D + D_expand).
5. SVD the expanded bond matrix. The null-space vectors now provide physically relevant non-zero singular values that push the smallest eigenvalues above the noise floor naturally.
6. Truncate back to D_max per `DMRGConfig`.
...
pub fn expand_bond_subspace<T: Scalar, Q: BitPackable, B: LinAlgBackend>(
    ...
    // 1. Compute null-space projector: P = I - A_L · A_L†
    // 2. Compute residual: |R⟩ = P · H_eff · |ψ_center⟩
    // 3. SVD of residual → take top `expansion_vectors` left singular vectors
...
| 5 | **Subspace expansion** (TDVP only) | tk-dmrg | Site tensors A_L/A_R padded with null-space vectors; bond matrix zero-padded and re-truncated. |
...
- **tk-core:** Round-trip shape permutations, stride arithmetic, arena allocation/reset safety. **MatRef adjoint round-trip:** `mat.adjoint().adjoint()` recovers original strides and conjugation flag. **PinnedMemoryTracker:** budget enforcement, try_reserve failure, Drop release.
...
- **tk-dmrg:** Heisenberg chain ground-state energy against reference snapshots (see §12.3), canonical form invariants. **Subspace expansion dimensional consistency:** verify A_L column count increases by D_expand; bond matrix shape matches; SVD re-truncation produces valid MPS.
...
| tk-core bloat invalidating compile cache | Medium | Strict scope discipline: only memory allocation, metadata, Scalar trait, MatRef/MatMut, error types |
| PyO3 GIL deadlock from Rayon workers | High | AtomicBool cancellation flag; GIL re-acquisition only from dedicated monitor thread; Rayon workers never touch the GIL |
...
|:------|:-------|:-------------|
| **Phase 1** | Months 1–3 | tk-core (strict scope, MatRef with conjugation flag, PinnedMemoryTracker), tk-symmetry (U(1), Z₂, BitPackable, PackedSectorKey), tk-linalg (DeviceFaer with conjugation-aware GEMM, gesdd default, regularized pseudo-inverse, LPT-scheduled block_gemm). >90% test coverage. SVD + conjugation benchmarks. iai CI integration. |
| **Phase 2** | Months 4–6 | tk-contract (DAG, greedy optimizer, conjugation flag propagation), tk-dsl (Index, OpSum, hamiltonian!{} macro — AST generation only). OpSum→MPO compilation in tk-dmrg. Heisenberg ground state matches reference snapshots. |
...
| **Phase 4** | Months 10–12 | tk-dmft (TDVP with Tikhonov + subspace expansion, linear prediction, Chebyshev, DMFT loop). tk-python (GIL release, AtomicBool cancellation, rust-numpy). StorageDevice generalization. MPI Mode B with pinned-memory topology query. Bethe lattice validation. |
| **Phase 5** | Months 13+ | DeviceCuda + PinnedArena + CudaArena (budget-managed). SU(2) non-Abelian. TreeSA/DP optimizers. Multi-orbital DMFT. MPI Mode A if needed. Community release. PyPI wheel builds (pure-Rust backends only). |
...
The v3.0 revision addresses five critical issues identified in the second external review: (1) the subspace expansion algorithm now correctly operates on site tensors A_L/A_R rather than the dimensionally incompatible bond matrix C; (2) block-sparse Rayon dispatch uses LPT scheduling to prevent long-tail thread starvation from binomial sector-size distributions; (3) a global atomic pinned-memory budget tracker with MPI-aware topology queries prevents multi-rank OOM crashes while maintaining graceful fallback to pageable memory; (4) `MatRef` carries a lazy `is_conjugated` flag that propagates through the contraction engine to BLAS/faer micro-kernels, eliminating O(N) conjugation memory passes; and (5) Python signal checking uses `AtomicBool` cancellation instead of direct GIL re-acquisition from Rayon workers, preventing deadlocks.
