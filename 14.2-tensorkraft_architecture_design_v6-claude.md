...
*Version 6.0 — March 2026 | Status: Architectural Specification (Pre-Implementation)*
...
- **tk-dmft:** Spectral function sum rules, bath discretization accuracy, self-consistency loop convergence against Bethe lattice benchmarks. **Exponential windowing:** verify deconvolved A(ω) matches unwindowed A(ω) for gapped phases (η should be a no-op); verify metallic-phase G(t) prediction does not diverge with η > 0. **Soft D_max:** verify bond dimension does not oscillate between expansion and truncation; verify smooth decay back to D_max over `dmax_decay_steps`.
...
| Linear prediction ill-conditioning | High | SVD-regularized pseudo-inverse with noise floor cutoff; exponential windowing (η) for metallic G(t); mandatory Chebyshev cross-validation; Lorentzian deconvolution post-FFT |
...
The v6.0 revision addresses four issues from the fifth external review: (1) `LinAlgBackend` is parameterized at the trait level (`LinAlgBackend<T>`) instead of using per-method generics, restoring object safety and enabling `Box<dyn LinAlgBackend<f64>>` for dynamic dispatch; (2) the monitor thread shutdown sequence is moved inside the `py.allow_threads` closure, eliminating a GIL deadlock where the main thread held the GIL while waiting for the monitor thread to finish a `Python::with_gil` call; (3) the time-domain window function is corrected from a Gaussian exp(−ηt²) to an exponential exp(−η|t|), whose Fourier transform is a Lorentzian — making the formula, terminology, and deconvolution procedure mutually consistent; and (4) a stateful `TdvpDriver` struct with per-bond `expansion_age` tracking is introduced to support the soft D_max decay formula across time steps.

Across six revision cycles, the design has been hardened against: dimensional inconsistencies in TDVP subspace expansion, scaling violations in null-space projection, bond-dimension oscillation at truncation thresholds, Rust object-safety violations in backend traits, cyclic crate dependencies, GIL deadlocks (Rayon workers, monitor thread shutdown ordering) and thread leaks in Python bindings, pinned-memory exhaustion under multi-rank MPI, Rayon long-tail starvation from binomial sector distributions, O(N) conjugation memory bandwidth waste, Fourier transform errors in spectral windowing, linear prediction instability in metallic phases, arena lifetime conflicts with persistent state, monomorphization-driven compile-time explosion, SVD gauge freedom in cross-backend tests, and FFI linker collisions between vendor BLAS libraries.
