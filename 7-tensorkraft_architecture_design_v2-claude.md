...
*Version 2.0 — March 2026 | Status: Architectural Specification (Pre-Implementation)*
...
- **tk-linalg:** Trait-based linear algebra backend abstraction defaulting to faer (dense) and oxiblas (sparse), swappable via feature flags. Includes regularized pseudo-inverse for numerically stable gauge restoration.
- **tk-contract:** DAG-based contraction engine with separated path optimization and execution phases.
- **tk-dsl:** Ergonomic API with intelligent indices, an `OpSum` builder pattern, and a `hamiltonian!{}` proc-macro that generates uncompressed operator sums as compile-time boilerplate reduction; actual MPO compression is a runtime operation delegated to `tk-dmrg`.

Additionally, integration crates **tk-dmrg** and **tk-dmft** implement the full DMRG sweep algorithm, iterative eigensolvers (Lanczos/Davidson, written in-house), MPO compilation from `OpSum`, time-evolution methods (TDVP with Tikhonov-regularized gauge shifts and subspace expansion, TEBD as fallback), and the DMFT self-consistency loop. A **tk-python** crate provides PyO3 bindings with explicit GIL management and zero-copy NumPy interop.
...
|:------|:---------------|:-----------|
| **tk-core** | Tensor shape/stride metadata, contiguous storage buffers, arena allocators, TensorCow (Copy-on-Write), element-type generics, error types | *(none — leaf crate)* |
...
| **tk-linalg** | LinAlgBackend trait, SVD/EVD/GEMM dispatch (gesdd default), regularized pseudo-inverse, DeviceFaer and DeviceOxiblas implementations, Rayon-parallel element-wise ops | tk-core, tk-symmetry |
...
| **tk-dsl** | Index struct with unique IDs and prime levels, `hamiltonian!{}` proc-macro (generates `OpSum` AST only), OpSum builder, Lattice trait, snake-path mappers | tk-core, tk-symmetry |
| **tk-dmrg** | MPS/MPO types with typestate canonicality, `OpSum → MPO` SVD compression, two-site sweep engine, in-house Lanczos/Davidson/Block-Davidson eigensolvers, SVD truncation | tk-core, tk-symmetry, tk-linalg, tk-contract, tk-dsl |
| **tk-dmft** | Anderson Impurity Model mapping, bath discretization (Lanczos tridiagonalization), TDVP time evolution (with Tikhonov regularization and subspace expansion), TEBD fallback, linear prediction, Chebyshev expansion, DMFT self-consistency loop | tk-dmrg (and transitively all) |
| **tk-python** | PyO3/maturin bindings with explicit GIL release, signal-checking callbacks, zero-copy NumPy via rust-numpy | tk-dmft |
...
**Scope discipline:** `tk-core` is the leaf crate upon which the entire workspace depends. To prevent compilation-cache invalidation cascading through the workspace, `tk-core` is strictly limited to: memory allocation (`SweepArena`), dimensional metadata (`TensorShape`, `TensorStorage`, `TensorCow`), the `Scalar` trait hierarchy, and shared error types. Mathematical operations on tensors (addition, trace, contraction) belong in `tk-linalg` or `tk-contract`.
...
pub struct SweepArena {
    inner: bumpalo::Bump,
    /// When true, arena memory is allocated via cudaMallocHost for
    /// DMA-capable host-device transfers. See §10.2.
...
**Capacity note:** With 8 tensor legs and 8 bits per quantum number (the U(1) default), bit-packing fits in exactly 64 bits. For models requiring larger charge ranges or higher-rank tensors (e.g., multi-orbital Hubbard with U(1)\_charge ⊗ U(1)\_spin, rank > 8), the `PackedSectorKey` can be promoted to `u128`, giving 16 bits per leg at rank 8 or supporting rank 16 at 8 bits per leg.
...
### 7.5 Python Bindings: GIL Management & Zero-Copy NumPy
...
impl PyDmftLoop {
    pub fn solve(&mut self, py: Python<'_>) -> PyResult<PySpectralFunction> {
        // Release the GIL: Python threads, signal handlers, and
        // Jupyter UI remain responsive during the multi-hour solve.
        let result = py.allow_threads(|| {
            match &mut self.inner {
                DmftLoopVariant::RealU1(solver) => solver.solve(),
                DmftLoopVariant::ComplexU1(solver) => solver.solve(),
                // ...
            }
        });
        Ok(PySpectralFunction::from(result))
    }
}
```

**Graceful `Ctrl+C` handling:** Releasing the GIL allows Python to *register* `SIGINT`, but does not force the Rust thread to stop. The `DMRGEngine` accepts an optional cancellation callback invoked at the end of each sweep step:

```rust
// Inside PyDmftLoop::solve:
let result = py.allow_threads(|| {
    solver.solve_with_callback(|| {
        // Briefly re-acquire GIL to check for pending signals
        Python::with_gil(|py| py.check_signals())
    })
})?; // '?' propagates KeyboardInterrupt back to Python
```
...
**Strategy 2 — Subspace Expansion (algorithmic growth):**
...
The subspace expansion protocol (2-site-to-1-site dynamic TDVP):

1. At the `BondCentered` step, compute the Hamiltonian residual: |R⟩ = H_eff|ψ⟩ − E|ψ⟩.
2. Mix a fraction α (typically ≈ 1e-4) of the local 2-site action of H_eff into the current bond matrix.
3. SVD the *expanded* matrix. The injected residual vectors populate the null space with physically relevant, non-zero singular values.
4. Truncate back to D_max per `DMRGConfig`.

This pushes the smallest singular values above the noise floor naturally, stabilizing C⁻¹ while allowing bond dimension growth.
...
pub struct TdvpStabilizationConfig {
    ...
    /// Mixing parameter α for subspace expansion.
...
{
    /// Run DMRG sweeps with an optional per-step callback.
    /// The callback is invoked at the end of each sweep step and may
    /// return Err to signal early termination (e.g., Python Ctrl+C).
    pub fn run_with_callback<F>(
        &mut self,
        callback: Option<F>,
    ) -> Result<T::Real, SolverError>
    where F: Fn() -> Result<(), SolverError>
    {
        // ... sweep loop with periodic callback invocation
    }
}
```
...
TDVP integration is stabilized via the dual Tikhonov + subspace expansion strategy described in §8.1.1.
...
where T: Scalar<Real = f64>, Q: BitPackable, B: LinAlgBackend
{
    ...
    /// Variant with per-sweep callback for Python signal checking.
    pub fn solve_with_callback<F>(&mut self, callback: F) -> Result<SpectralFunction, SolverError>
    where F: Fn() -> Result<(), SolverError>
    {
        // Same loop but passes callback through to DMRGEngine::run_with_callback
    }
}
```
...
#### 10.2.1 Pinned Memory for DMA Transfers
...
When the `backend-cuda` feature is active, `SweepArena` and `HostDevice` allocations use `cudaMallocHost` for pinned memory, enabling direct DMA transfers:

```rust
#[cfg(feature = "backend-cuda")]
pub struct PinnedArena {
    /// Memory allocated via cudaMallocHost: page-locked, DMA-capable.
    /// Bypasses the hidden double-copy in standard cudaMemcpy.
    base: *mut u8,
    capacity: usize,
    offset: AtomicUsize,
}
...
#[cfg(feature = "backend-cuda")]
impl SweepArena {
    /// Create an arena backed by pinned host memory.
    pub fn new_pinned(capacity: usize) -> Self {
        SweepArena {
            inner: PinnedArena::new(capacity),
            pinned: true,
        }
    }
}
...
#### 10.2.2 DeviceCuda Implementation
...
#[cfg(feature = "backend-cuda")]
impl LinAlgBackend for DeviceCuda { /* dispatch to cuBLAS, cuSOLVER (gesdd default) */ }
```

#### 10.2.3 Stream-Aware DAG Execution
...
#### 10.2.4 GPU Performance Considerations
...
|:----------|:------|:-----|:------|:------|
| **CUDA (single-node GPU)** | New DeviceCuda + StorageDevice generalization + PinnedArena | Medium | Phase 5 | High |
...
|:------|:--------|:--------|
| **faer** | tk-linalg | Dense SVD (gesdd/gesvd), EVD, QR, LU; multithreaded cache-optimized |
...
- **tk-linalg:** SVD reconstruction error < machine epsilon, eigenvector orthogonality, GEMM reference comparison. **Regularized pseudo-inverse:** verify Tikhonov formula against analytically known cases.
- **tk-contract:** Path FLOP estimates vs brute-force, result equivalence across optimizers.
- **tk-dmrg:** Heisenberg chain ground-state energy against reference snapshots (see §12.3), canonical form invariants.
- **tk-dmft:** Spectral function sum rules, bath discretization accuracy, self-consistency loop convergence against Bethe lattice benchmarks.
...
**Local bare-metal (Criterion.rs):** Tracks wall-clock time for dense SVD at D = 100, 500, 1000, 2000; block-sparse GEMM with U(1) sectors; full DMRG sweep (N=100, D=200); contraction path optimization for 10/20/50-tensor networks. Run only on dedicated developer machines or self-hosted CI runners.
...
| FLOP-only path optimization | Medium | Composite CostMetric (α·FLOPs + β·Bytes_Moved) propagates stride info through candidate trees; penalizes paths requiring explicit transposes |
...
| TDVP projector-splitting instability | High | Tikhonov-regularized pseudo-inverse (§5.1) + subspace expansion (§8.1.1); TdvpStabilizationConfig with tunable δ and α |
| TDVP entanglement growth bottleneck | High | Subspace expansion injects residual vectors to populate null space; adaptive 1-site/2-site switching |
...
| tk-core bloat invalidating compile cache | Medium | Strict scope discipline: only memory allocation, metadata, Scalar trait, error types |
| PyO3 GIL starvation | High | Explicit GIL release via py.allow_threads; callback-based signal checking; zero-copy NumPy via rust-numpy |
...
| GPU host-device bandwidth halved by pageable memory | Medium | PinnedArena via cudaMallocHost; SweepArena configurable for pinned allocation when backend-cuda active |
...
|:------|:-------|:-------------|
| **Phase 1** | Months 1–3 | tk-core (strict scope), tk-symmetry (U(1), Z₂, BitPackable, PackedSectorKey), tk-linalg (DeviceFaer, gesdd default, regularized pseudo-inverse). >90% test coverage. SVD benchmarks. iai CI integration. |
| **Phase 2** | Months 4–6 | tk-contract (DAG, greedy optimizer), tk-dsl (Index, OpSum, hamiltonian!{} macro — AST generation only). OpSum→MPO compilation in tk-dmrg. Heisenberg ground state matches reference snapshots. |
| **Phase 3** | Months 7–9 | tk-dmrg (MPS typestates, two-site sweep, in-house Lanczos/Davidson/Block-Davidson). N=100 Heisenberg at D=500. Gauge-invariant test macros. ITensor/TeNPy comparison via snapshot fixtures. |
| **Phase 4** | Months 10–12 | tk-dmft (TDVP with Tikhonov + subspace expansion, linear prediction, Chebyshev, DMFT loop). tk-python (GIL release, signal callbacks, rust-numpy). StorageDevice generalization. MPI Mode B. Bethe lattice validation. |
| **Phase 5** | Months 13+ | DeviceCuda + PinnedArena + CudaArena. SU(2) non-Abelian. TreeSA/DP optimizers. Multi-orbital DMFT. MPI Mode A if needed. Community release. PyPI wheel builds (pure-Rust backends only). |
...
The v2.0 revision addresses all identified structural risks: cyclic dependency prevention between `tk-dsl` and `tk-dmrg`, FFI linker collision guards, bit-packed sector keys for branch-free BLAS dispatch, Tikhonov-regularized TDVP gauge shifts with subspace expansion for entanglement growth, explicit GIL management for responsive Python integration, pinned-memory arenas for full-bandwidth GPU transfers, and a hardened testing strategy with gauge-invariant assertions, snapshot-based validation, and deterministic CI benchmarks.
