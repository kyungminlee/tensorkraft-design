...
*Version 8.0 — March 2026 | Status: Architectural Specification (Pre-Implementation)*
...
Additionally, integration crates **tk-dmrg** and **tk-dmft** implement the full DMRG sweep algorithm, iterative eigensolvers (Lanczos/Davidson, written in-house), MPO compilation from `OpSum`, time-evolution methods (TDVP with Tikhonov-regularized gauge shifts and site-tensor subspace expansion for entanglement growth, TEBD as fallback), and the DMFT self-consistency loop with Levinson-Durbin-based linear prediction and mandatory spectral positivity restoration. A **tk-python** crate provides PyO3 bindings with explicit GIL management, thread-safe `AtomicBool` cancellation with `mpsc`-guarded monitor thread lifecycle, and zero-copy NumPy interop.
...
| **tk-linalg** | LinAlgBackend trait (conjugation-aware GEMM), SVD/EVD dispatch (gesdd default), regularized pseudo-inverse, DeviceFaer and DeviceOxiblas implementations, partitioned LPT-scheduled block-sparse dispatch, Rayon-parallel element-wise ops | tk-core, tk-symmetry |
...
| **tk-dmft** | Anderson Impurity Model mapping, bath discretization (Lanczos tridiagonalization), TDVP time evolution (with Tikhonov regularization and site-tensor subspace expansion), TEBD fallback, linear prediction (Levinson-Durbin default, SVD fallback), spectral positivity restoration, Chebyshev expansion, DMFT self-consistency loop, pinned-memory budget initialization for MPI | tk-dmrg (and transitively all) |
...
impl PartitionedLptConfig {
    ...
    pub fn calibrate(backend: &impl LinAlgBackend<f64>) -> Self {
        ...
        PartitionedLptConfig { blas_flop_threshold: threshold, n_cores }
    }
...
where DeviceFaer: LinAlgBackend<T>
{
    fn block_gemm(
        ...
        let mut results = Vec::with_capacity(heavy_tasks.len() + light_tasks.len());

        // 3a. Execute heavy tasks with full BLAS threading.
        ...
        // 3b. Execute long tail with Rayon (single-threaded BLAS).
...
The four-phase design (generate → LPT sort → partitioned dispatch → restore invariant) achieves optimal core saturation for both the heavy head and the fragmented tail. The serialization barrier between phases 3a and 3b is acceptable because in the common case the heavy tasks dominate wall time — the light tail is a rounding error. For edge cases where many medium-sized tasks cluster near the threshold, a more sophisticated weighted task-stealing pool could be introduced in a future revision, but the two-phase partition is the correct design for v8.0.
...
where T: Scalar<Real = f64>, Q: BitPackable, B: LinAlgBackend<T>
{
    pub fn solve(&mut self) -> SpectralFunction {
        loop {
            ...
            // Mandatory: deconvolve, then restore positivity.
...
| Thread pool oversubscription (middle-ground pathology) | High | Partitioned LPT dispatch (§5.3): tasks split at calibrated `blas_flop_threshold`; heavy head runs with full BLAS multithreading; light tail cleared by Rayon with single-threaded BLAS. Optional startup microbenchmark calibrates threshold to host machine |
...
| **Phase 4** | Months 10–12 | tk-dmft (TDVP with Tikhonov + subspace expansion + soft D_max, linear prediction with Levinson-Durbin default + exponential windowing + spectral positivity restoration, Chebyshev, DMFT loop). tk-python (GIL release, AtomicBool cancellation, rust-numpy). StorageDevice generalization. MPI Mode B with pinned-memory topology query. Bethe lattice validation (insulating + metallic phases; verify positivity restoration on metallic spectra). |
| **Phase 5** | Months 13+ | DeviceCuda + PinnedArena + CudaArena (budget-managed); single-thread DAG submission. SU(2) non-Abelian. TreeSA/DP optimizers. Fermionic swap gates for tree/PEPS (`fermionic-swap` feature flag). Multi-orbital DMFT. MPI Mode A if needed. Community release. PyPI wheel builds (pure-Rust backends only). |
...
The v8.0 revision closes the remaining systems-engineering gaps identified across eight review cycles: (1) the binary thread-pool regime switch that abandoned either BLAS multithreading or Rayon parallelism in the most common DMRG workload has been replaced with a partitioned LPT dispatcher that handles both heavy and fragmented sectors optimally, with an optional startup microbenchmark for machine-specific calibration; (2) the `hamiltonian!{}` proc-macro now specifies span hygiene guarantees and IDE-friendly diagnostic error reporting, matching the ergonomic standard physicists expect from a domain-specific language; (3) the linear prediction subsystem defaults to the Levinson-Durbin recursion for its Toeplitz system and enforces mandatory spectral positivity restoration post-deconvolution, preventing negative spectral weights from crashing the bath discretization step; and (4) the GPU submission model is clarified as single-thread for Phase 5, with a dedicated submission queue documented for Phase 5+ multi-GPU extensions.

Across eight revision cycles, the design has been hardened against: dimensional inconsistencies in TDVP subspace expansion, scaling violations in null-space projection, bond-dimension oscillation at truncation thresholds, Rust object-safety violations in backend traits, cyclic crate dependencies, GIL deadlocks (Rayon workers, monitor thread shutdown ordering) and thread leaks in Python bindings, pinned-memory exhaustion under multi-rank MPI, silent pinned-memory performance cliffs, NUMA-blind GPU allocations, Rayon long-tail starvation from binomial sector distributions, thread-pool oversubscription from binary regime switching, O(N) conjugation memory bandwidth waste, Fourier transform errors in spectral windowing, deconvolution noise amplification, negative spectral weight from deconvolution ringing, SVD silent inaccuracy in divide-and-conquer algorithms, SU(2) fusion-rule multiplicity in task generation, linear prediction instability in metallic phases, MPI barrier load imbalance, arena lifetime conflicts with persistent state, monomorphization-driven compile-time explosion, SVD gauge freedom in cross-backend tests, FFI linker collisions between vendor BLAS libraries, proc-macro variable shadowing, and cryptic DSL error diagnostics.
