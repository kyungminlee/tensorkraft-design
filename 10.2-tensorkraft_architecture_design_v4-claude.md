...
*Version 4.0 — March 2026 | Status: Architectural Specification (Pre-Implementation)*
...
7. Truncate back to D_max per `DMRGConfig`.
...
The v4.0 revision addresses two edge cases introduced by the v3.0 fixes: (1) the subspace expansion null-space projection now uses matrix-free sequential projection (O = A_L†·|R⟩, |R_null⟩ = |R⟩ − A_L·O) at O(dD²) cost, avoiding the O(d²D³) blowup of explicit projector construction; and (2) the Python signal-monitor thread uses `mpsc::channel` with `recv_timeout` for lifecycle management, guaranteeing clean thread termination when the solver completes, panics, or is cancelled, with `JoinHandle::join()` preventing dangling threads during CPython interpreter teardown.

Across four revision cycles, the design has been hardened against: dimensional inconsistencies in TDVP subspace expansion, scaling violations in null-space projection, cyclic crate dependencies, GIL deadlocks and thread leaks in Python bindings, pinned-memory exhaustion under multi-rank MPI, Rayon long-tail starvation from binomial sector distributions, O(N) conjugation memory bandwidth waste, SVD gauge freedom in cross-backend tests, and FFI linker collisions between vendor BLAS libraries.
