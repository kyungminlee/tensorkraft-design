...
*Version 8.1 — March 2026 | Status: Architectural Specification (Pre-Implementation)*
...
**Flat-buffer block storage refactor (Phase 4):** The current layout stores each block as an independent `DenseTensor<T>`, which is adequate for CPU-only Phases 1–3 (blocks are large enough that per-allocation overhead is negligible). However, for GPU transfers in Phase 5, hundreds of individually-allocated blocks would require either hundreds of small `cudaMemcpyAsync` calls (terrible PCIe utilization) or manual gathering into a staging buffer. In Phase 4, the `BlockSparseTensor` storage should be refactored to a single contiguous flat buffer with an offset table:

```rust
/// Phase 4 refactored block storage: all sector data in one contiguous allocation.
/// Enables single-DMA GPU transfer of the entire tensor.
pub struct FlatBlockStorage<T: Scalar> {
    /// Single contiguous buffer containing all sector blocks back-to-back.
    data: Vec<T>,
    /// Byte offset and shape of each sector block within `data`.
...
This is a non-breaking refactor because the `SparseLinAlgBackend` trait takes `&BlockSparseTensor` as an opaque input — no external code accesses block storage internals directly. The key invariant (sorted `sector_keys` with O(log N) binary search) is preserved; only the backing memory layout changes from scattered heap allocations to a single arena-friendly buffer.
...
**Eigensolver memory ownership:** The Krylov basis vectors themselves are **heap-allocated** (`Vec<Vec<T>>`) and owned by the eigensolver instance, *not* arena-allocated. At D = 2000 with `max_krylov_dim = 20`, the Krylov workspace is ~640 MB (20 × 2000² × 8 bytes). If these vectors lived in the `SweepArena`, they would inflate the arena's high-water mark until the sweep step ends, even though they are only needed during the eigensolver phase. By using standard heap allocation with normal Rust drop semantics, the Krylov workspace is freed immediately when the eigensolver returns — *before* the SVD phase begins — reducing peak memory by up to 640 MB. The `SweepArena` remains reserved for contraction temporaries inside the matvec closure (which *are* recycled within the step) and for the SVD workspace that follows.
...
fn restore_positivity(
    ...
    SpectralFunction { omega: spectral.omega.clone(), values: clamped }
}
```
...
**Three-way LPT partition for block-sparse GEMM:** When `backend-cuda` is active, the partitioned LPT scheduler (§5.3) performs a three-way split: tasks above `gpu_flop_threshold` are submitted as a batched `cublasDgemmBatched` call (amortizing kernel launch overhead across all GPU-tier sectors in a single launch); tasks between `gpu_flop_threshold` and `blas_flop_threshold` run on CPU with full multithreaded BLAS; and the fragmented tail of tiny sectors runs on CPU via Rayon with single-threaded BLAS. The fragmented tail *never* touches the GPU — dispatching hundreds of 10×10 or 50×50 GEMMs to individual CUDA streams would saturate the PCIe queue and incur launch overhead far exceeding the compute time.
...
- **tk-linalg:** SVD reconstruction error < machine epsilon, eigenvector orthogonality, GEMM reference comparison. **Conjugation-aware GEMM:** verify C = A†·B matches explicit conjugation + transpose + multiply for random complex matrices. **Regularized pseudo-inverse:** verify Tikhonov formula against analytically known cases. **Partitioned LPT scheduling:** verify FLOP-descending order after sort; verify partition split at threshold; verify both heavy and light tasks produce correct results; assert all sectors present in output. **SVD residual guard:** verify `debug_assert!` fires on synthetically corrupted singular values; verify it passes on well-conditioned matrices. **BLAS crossover calibration:** verify microbenchmark produces a reasonable threshold (not 0, not infinity).
...
| Negative spectral weight crashing bath discretization | High | Mandatory positivity restoration (§8.4.2): clamp to ε_floor, L₁ renormalize to preserve sum rule. Diagnostic warning if clamped weight exceeds 5% of total, indicating misconfigured deconvolution parameters |
| Block storage heap fragmentation (GPU transfers) | Medium | Phase 1–3: `Vec<DenseTensor<T>>` per-block is adequate for CPU-only. Phase 4: refactor to flat contiguous buffer with offset table (`FlatBlockStorage`) for single-DMA GPU transfer. Non-breaking because `SparseLinAlgBackend` does not expose storage internals (§4.2) |
| GPU kernel launch overhead on fragmented sectors | Medium | Three-way LPT partition (§5.3): tasks below `gpu_flop_threshold` never touch GPU. GPU-tier tasks submitted as batched `cublasDgemmBatched`. Fragmented tail runs CPU-only via Rayon (§10.2.5) |
...
| Eigensolver Krylov vectors inflating arena high-water mark | Medium | Krylov vectors heap-allocated (`Vec<Vec<T>>`), owned by eigensolver, dropped before SVD phase. Contraction temporaries inside matvec remain arena-allocated. Reduces peak memory by up to 640 MB at D=2000 (§8.2) |
...
The v8.1 revision addresses nine points from the ninth and tenth external reviews: (1) block-sparse tensor storage is documented for a Phase 4 flat-buffer refactor enabling single-DMA GPU transfers without breaking the `SparseLinAlgBackend` trait boundary; (2) the BLAS thread-count global state mutation is documented with serialization guarantees, `mkl_set_num_threads_local` as the preferred path on newer MKL, and faer's per-call parallelism as the default immune alternative; (3) the partitioned LPT scheduler gains a three-way GPU/CPU-BLAS/Rayon partition when `backend-cuda` is active, preventing kernel launch overhead from dominating tiny sector GEMMs; (4) the path optimizer gains an optional `max_memory_bytes` constraint for future PEPS/tree TNS extensions; (5) subspace expansion SVD is performed per symmetry sector, guaranteeing flux-preserving expansion vectors by construction; (6) eigensolver Krylov vectors are heap-allocated and dropped before SVD, reducing peak memory by up to 640 MB at D=2000; (7) the DMFT loop gains adaptive TDVP/Chebyshev solver selection based on the entanglement spectrum gap, automatically promoting Chebyshev to primary for metallic phases; (8) in-house eigensolvers are cross-validated against ARPACK-NG in integration tests; and (9) NUMA placement verification is recommended for multi-socket single-GPU deployments.

Across ten revision cycles, the design has been hardened against: dimensional inconsistencies in TDVP subspace expansion, scaling violations in null-space projection, symmetry-sector corruption from dense expansion SVD, bond-dimension oscillation at truncation thresholds, Rust object-safety violations in backend traits, cyclic crate dependencies, GIL deadlocks (Rayon workers, monitor thread shutdown ordering) and thread leaks in Python bindings, pinned-memory exhaustion under multi-rank MPI, silent pinned-memory performance cliffs, NUMA-blind GPU allocations, Rayon long-tail starvation from binomial sector distributions, thread-pool oversubscription from binary regime switching, BLAS global thread-count race conditions, GPU kernel launch overhead on fragmented sectors, O(N) conjugation memory bandwidth waste, Fourier transform errors in spectral windowing, deconvolution noise amplification, negative spectral weight from deconvolution ringing, unreliable TDVP linear prediction for metallic phases, SVD silent inaccuracy in divide-and-conquer algorithms, SU(2) fusion-rule multiplicity in task generation, linear prediction instability in metallic phases, eigensolver Krylov workspace inflating arena high-water marks, memory blow-up in contraction path optimization, MPI barrier load imbalance, arena lifetime conflicts with persistent state, monomorphization-driven compile-time explosion, SVD gauge freedom in cross-backend tests, FFI linker collisions between vendor BLAS libraries, block storage heap fragmentation for GPU transfers, proc-macro variable shadowing, and cryptic DSL error diagnostics.
